...
function checkStatus() {
  return new Promise(async (resolve) => {
    ...
    if (result.includes('Changes')) {
      resolve(STATUS.COMMIT)
    }
    if(result.includes('to publish your local commits')) {
      resolve(STATUS.PUSH)
    }
    if(result.includes('branch is up to date')) {
...
module.exports = { log, preLog, checkPull, exec, prompt, checkBranchExist, checkStatus, checkHasUpstream };
